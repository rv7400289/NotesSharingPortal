...
def userlogin(request):
    ...
    if request.method == 'POST':
        ...
        user = authenticate(username=u, password=p)
                                                                 ...
                                                                 #This section is used for validation
        try:
            if user:
                login(request, user)
                error = 'no'
...
def signup1(request):
    error=""
    if request.method=='POST':
        f=request.POST['firstName']          # to store the data of signup
...
def upload_notes(request):
    ...
    error=""
    if request.method=='POST':
        b=request.POST['branch']          # to store the data of notes
...
def edit_profile(request):
    ...
    user=User.objects.get(id=request.user.id) # here we get the user data stored in user module
    data = Signup.objects.get(user=user) # here we get the signup data stored in signup module
    error=False
    if request.method=='POST':
...
def view_mynotes(request):
    ...
    if not request.user.is_authenticated: # if user is not login then send it to first login page
        return redirect('login')

    user=User.objects.get(id=request.user.id) # here we get the user data stored in user module
    notes = Notes.objects.filter(user=user) # here we get the signup data stored in signup module
    d={'notes':notes,}
...
def view_users(request):
    ...
    users=Signup.objects.all()# here we get the user data stored in user module
    d={'users':users}
